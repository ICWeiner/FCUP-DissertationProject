...
def start(proxmox_host, session, vm_proxmox_id):
    try:
        response = session.post(f'{proxmox_base_uri(proxmox_host)}/nodes/{constants.proxmox_node_name}/qemu/{vm_proxmox_id}/status/start')
        response.raise_for_status()
...
def stop(proxmox_host, session, vm_proxmox_id):
    try:
        response = session.post(f'{proxmox_base_uri(proxmox_host)}/nodes/{constants.proxmox_node_name}/qemu/{vm_proxmox_id}/status/stop')
        response.raise_for_status()
