...
its type (e.g. Cisco router, Linux VM have different syntax for a traceroute). This effort was made to ensure support for a 
...
\section{Future Work}
    ...
    permission system. At present,\ac{pve} assumes administrative access if authenticated, which is suitable for development 
    but not for production deployment, with only the web application doing role checking. A more fine-grained approach—where 
    roles such as "student" and "teacher" are mapped to restricted scopes in\ac{pve}—remains an important area for future work.
