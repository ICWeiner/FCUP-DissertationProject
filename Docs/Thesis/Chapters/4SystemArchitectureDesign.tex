...
\section{System Architecture Overview}
    ...
    \begin{itemize}
        ...
        of this component two types of users exist, students and teachers, with the latter having access to more features than former. This component 
...
\section{Proxmox VE}
    ...
    \subsection{Proxmox VE Limitations}
        ...
        During development, we encountered some challenges when interfacing programmatically with\ac{pve}. One of the most significant 
        ...
        including exponential backoff retry logic and strict web application concurrent request limiting when interacting with\ac{pve} to maintain 
        system stability.
        ...
        removed, they accumulated as unused entries in storage, eventually leading to a noticeable performance regression. This specific limitation 
    ...
    \subsection{Proxmox VE Firewall}
        ...
        The\ac{pve} host-level firewall provides essential features for securing student work environments, during examination 
...
\section{GNS3}
    ...
    and configure a new\ac{vm} on the virtualization infrastructure, then proceed to install and set up the\ac{gns3} environment. The final preparation 
    step involves importing all necessary device images, including routers, switches, and other equipment that will be available for student exercises.
    This is done directly on the virtualization infrastructure as this will be the base image for all work environments hosted on it.
    ...
    cluster, allowing the platform to accommodate growing numbers of users. This will however require additional work on the developed orchestration 
    capabilities to be able to distribute the\ac{vm}s in the cluster, as\ac{pve} does not have automatic load balancing capabilities, only being able to 
    ensure uptime.
...
\section{High-level architecture}
    The system architecture is divided into five main components: the interface component, which provides user access via both server-side rendered pages 
    ...
    \subsection{Evaluation component}
        ...
        using a ping command. One of these devices is a Linux host, while the other is a Cisco\ac{ios} device. Before interacting with either, 
        ...
        allowing support for a wide range of configuration checks. For example, the included \texttt{PingModule} implements platform-specific variants 
        of the ping command and corresponding response interpretation methods. This design promotes code reuse while allowing easy extension for new 
        test types, as developers can create additional modules by simply extending the base class and implementing the required platform-specific methods.
        ...
        ping command for cisco routers versus the Linux \texttt{ping -c} syntax for Linux hosts, as was exemplified earlier. The module captures and 
        ...
        results. For connectivity tests like ping, the interpretation logic can be configured to allow some tolerance, such as classifying a ping 
        ...
        Seen in Figure~\ref{fig:system-diagram} we can see how the elements discussed so far interact with each other.
